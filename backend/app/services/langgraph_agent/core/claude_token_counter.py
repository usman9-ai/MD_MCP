...
|---|---|---|---|---|
| Sale Value | `Sale Value` | Secondary Sales Summary | REAL, measure | PKR. Always filter Flag + Type Of Sale before summing (§4). |
| Sale Unit | `Sale Unit` | Secondary Sales Summary | REAL, measure | Units sold. Same filter rules as Sale Value. |
| Flag | `Flag` | Secondary Sales Summary | STRING, dimension | `T` = target, `O` = operational target, blank/null = actual. Filter pattern in §4.6. |
...
| Company | `CMP` | Secondary Sales Summary | STRING, dimension | Stores **full legal names**, not abbreviations — see §4.7 for the exact strings and mapping. Confirmed field — do not use `Cmpid`, `Cmp Code1`, or `Alt Company Code` for this. |
...
| Product | `Product` | Secondary Sales Summary | STRING, dimension | Default field for "top-selling products." |
...
| Data source LUID | `<<resolve via list-datasources tool at session start>>` | — | — | Not present in a metadata export. |
...
4. **Focused Brands** (`FOCUSED_BRAND`) — flags certain product groups as "focused" per company per year (`Focus Flag`, `Prod Group`). Joined via a composite key (`Sk Grp Key` + `Cmpid`). Only pull this in when a question is specifically about focused-brand status.
...
- `Flag` blank/null → **actual sale value/units** (what really sold)
- `Flag = 'T'` → **target**
- `Flag = 'O'` → **operational target (OP)**

**Every query must explicitly filter `Flag` to exactly one of these three states.** Omitting it sums actuals + targets + OP together — a meaningless, inflated number. There is no valid business question that requires an unfiltered Flag.

Default: "what sold," "revenue," "units sold," "top products," "regional performance," etc. → actuals (Flag blank/null) unless the question is explicitly about targets (§4.2).

### 4.2 Target logic: Target (T) is default, OP (O) only on request
- Compute **Actual (Flag blank/null) vs. Target (Flag = 'T')** by default.
- Achievement % = `SUM(Sale Value or Sale Unit, actual) / SUM(Sale Value or Sale Unit, Flag='T')`
- Switch the denominator to `Flag = 'O'` only if the user explicitly says "OP," "operational target," or similar — and say so plainly in the answer when you do.
- **Do not use the separate `Target Value` / `Target Unit` fields for this calculation** — see §5.
...
### 4.6 Concrete filter patterns (copy-paste ready)
`Flag`'s "actual" state is blank/null rather than a labeled member, so the most reliable pattern is a `SET` filter that excludes the two labeled states rather than trying to filter for null directly:
...
## 5. FIELDS EXCLUDED FROM DEFAULT KPI LOGIC

Two pairs of fields exist in the fact table that look like they might be shortcuts for the KPIs above. They are **not** — do not use them in any default calculation:

- **`Target Value` / `Target Unit`** — separate measure fields sitting alongside Sale Value/Sale Unit. **Confirmed rule: targets are computed exclusively via `Flag = 'T'` rows in `Sale Value`/`Sale Unit` (§4.2). Do not use these fields for Target Achievement or any default target figure.** If a user explicitly asks what the `Target Value`/`Target Unit` fields themselves contain, treat that as a distinct, labeled ad-hoc pull — don't blend it into the standard KPI.
- **`SALE_VALUE_PRIMARY` / `SALE_UNIT_PRIMARY`** — business definition **not yet confirmed** (possibly "primary sales," i.e., company→distributor, as distinct from this table's core "secondary sales" figures — but unconfirmed). **Do not use these in any default KPI.** If a user asks about them, or asks about "primary sales" specifically, say the field exists but its business definition hasn't been confirmed yet, and ask before reporting a number from it.

Other available-but-secondary measures in the fact table (`Bonus`, `Expired`, `Returns`, `Discount`, `TP`, `Cpvactual`, `With And Without Bonus`) are fine to query if a user explicitly asks about them, but don't fold them into Sale Value/Sale Unit/Target Achievement by default.
...
| KPI | Definition |
|---|---|
| **Sale Value** | `SUM(Sale Value)`, Flag = actual, Type Of Sale as scoped. PKR. |
| **Sale Unit** | `SUM(Sale Unit)`, Flag = actual, Type Of Sale as scoped. Units. |
| **Target Achievement %** | `SUM(actual) / SUM(Flag='T')` by default (§4.2), by value or unit as asked. |
| **Top-selling products (by value)** | Rank `Product` by `SUM(Sale Value)`, actual, Type Of Sale as scoped — use a `TOP` filter (see §9). |
| **Top-selling products (by unit)** | Rank `Product` by `SUM(Sale Unit)`, actual, Type Of Sale as scoped. |
...
4. **Apply date logic** via `Gregorian Date` on Dim Time (§4.5) if time is involved.
...
- [ ] Does the query filter `Flag` to exactly one state (actual / T / O)?
...
- [ ] Have I distinguished `Saletype` (channel) from `Type Of Sale` (GEO/National) correctly?
...
```json
{
  ...
  "query": {
    ...
    "filters": [
      ...
      { "field": { "fieldCaption": "Product" }, "filterType": "TOP", "howMany": 5, "fieldToMeasure": { "fieldCaption": "Sale Value", "function": "SUM" }, "direction": "TOP" }
    ]
  }
}
```
