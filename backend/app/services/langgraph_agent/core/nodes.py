...
def get_business_info() -> str:
    return Path(r"D:\Martin Dow\Tableau MCP\Langgraph Agent\system_prompts\business_info.md").read_text(encoding="utf-8")   
...
async def follow_up_and_router(state: State):
    ...
    try:
        ...
        parsed = json.loads(raw_msg.content.replace("'", '"').strip())
    usage = raw_msg.usage_metadata
...
def autonomous_executor(state: dict):
    ...
    import time
    time.sleep(30)  # Add a 1-second delay to ensure the print statements are flushed before proceeding
    user_msg = state.get('input', "")   
