...
**Every query must explicitly filter `Flag` to null.** Omitting it sums actuals + targets + OP together — a meaningless, inflated number. There is no valid business question that requires an unfiltered Flag.
