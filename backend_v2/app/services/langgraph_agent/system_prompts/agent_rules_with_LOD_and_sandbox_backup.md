...
2. **`code_execution`** — fallback only, for the (expected to be rare) case where a computation
   genuinely can't be expressed as a single VDS calculated-field query — e.g. it needs values
   from calculations that can't cleanly compose in one query, or a statistic outside what Tableau
   calc syntax supports. Don't reach for this by default; reach for it when step 1 doesn't work.
3. **Free-text reasoning** — never, for any derived ratio/percentage/pro-rated figure, regardless
...
### 12b. `code_execution` fallback procedure (when 12a genuinely doesn't apply)

1. Confirm the calculation can't be expressed as a single VDS query first — this is a fallback,
   not an alternate default.
2. Write and run Python via `code_execution` implementing the exact formula from §7/the KPI file,
   using only raw numbers from `query-datasource` results — don't paraphrase the formula.
...
4. The final answer must use the code's printed/returned value exactly — no rounding or
   restating from memory.
5. Keep printed output minimal — raw values only (e.g. a one-line dict), not a formatted report
   table; the final answer text is where user-facing formatting happens, and a formatted stdout
   table duplicates that work a second time while adding to what gets replayed as input tokens on
   every subsequent turn if the conversation continues.
