...
**`Stock` / `Vstock` carry real historical data and must always be filtered on `Gregorian Date`
— never summed unfiltered.** Default to the single latest available date only (same
latest-available-date value already used for the sales default, §6 — don't run a separate
`MAX(Gregorian Date)` probe against the stock table unless there's reason to think stock data
lags sales data) and report that as "current stock," disclosing the as-of date in the answer. Only
widen to a range/trend if the user explicitly asks for historical stock or a stock trend — this
table is no longer treated as a current-day-only snapshot, so that request is answerable, not a
"not available" case.
