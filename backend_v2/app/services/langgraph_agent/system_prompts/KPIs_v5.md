...
- **Distributor Stock** — **[CONFIRMED, with a scope caveat]**. `SUM(Stock)` (units) / `SUM(Vstock)`
  ...
  table holds **daily historical stock data**, not a current-day-only snapshot — it must always be
  filtered on `Gregorian Date`. Default behavior: filter to the single latest available date and
  report that as "distributor stock right now," disclosing the as-of date used. If the user asks
  for stock at a past date or a stock trend over time, this is answerable — filter/group
  `Gregorian Date` accordingly rather than saying historical stock isn't available. `Vstock`'s
  valuation basis (whether it's independently sourced or `Stock × Trade Price`) is still
  **[NEEDS CONFIRMATION]** — hedge on the currency/derivation of that number specifically, even
  though the field itself is now confirmed to exist and be usable.
...
- **Chemist Count** — `COUNTD(SK_CUSTOMER_KEY)`, filtered to rows with `Sale Value` (or `Sale Unit`) > 0
  in the period, Flag=null, Type Of Sale as scoped. Use `SK_CUSTOMER_KEY`, not `CUSTOMER_NAME` (names can
...
- **Daily chemist sales** — `SUM(Sale Value)` / `SUM(Sale Unit)` grouped by `Gregorian Date` (not
  `Dateofsale`) and `SK_CUSTOMER_KEY`/`CUSTOMER_NAME`, standard Flag/Type Of Sale filters. **[CONFIRMED]**
...
Current Stock (units) = Number of Stock units at the latest available date
```
- "Trailing 90 days" = the 90 calendar days ending at the latest available actual-data date
  (§6), not today's calendar date.
- `Stock` comes from the Distributor Stock table (metadata Table 5), which holds **daily
  historical data, not a current-day-only snapshot**. `Current Stock (units)` for this KPI must
  always be computed with an explicit `Gregorian Date` filter to the single latest available date
  — never an unfiltered `SUM(Stock)` over the whole join, which would blend historical days
  together into a meaningless number. Use the same latest-available-date value as the sales
  default (§6) unless stock is known to lag sales.
