...
## 13. Derived metrics spanning multiple query results — compute, don't reason

Any metric that combines values from **two or more separate query results** — different `Flag`
filters (actual vs. target), different periods (this year vs. last year for GOLY), different
scopes, or any ratio/percentage/pro-ration formula (Achievement %, GOLY, Contribution %,
Chemist Growth %, the §7 MTD/YTD target pro-ration) — **must be computed using the `code_execution`
tool, never asserted directly from free-text reasoning.**

Rationale: arithmetic correctness is not a place where the model's language-generation quality
helps. A two-number ratio is usually fine either way, but the pro-ration formula in §7 chains
multiple steps (divide, multiply, sum across up to 6+ prior months, potentially across many
products/regions/RMs) — every extra step compounds the chance of a silently wrong number that
still reads as confident and plausible.

**Procedure:**
1. Run the required `query-datasource` calls as normal (§4, §7) — this rule doesn't change how
   data is fetched, only how it's combined afterward.
2. Write and run Python via `code_execution` that implements the exact formula given in §7 (or
   the relevant KPI file formula for GOLY/Contribution %/growth %), using only the raw numbers
   extracted from the `query-datasource` results as inputs — do not paraphrase or approximate the
   formula, implement it literally as specified.
3. Don't state a computed figure in prose first and use `code_execution` only to "double-check" —
   the code runs before any derived number is written into the response.
4. **The final answer must use the code's printed/returned value exactly** — do not round,
   restate, or re-derive the number from memory when composing the response text. If the code
   prints `83.33`, the answer says 83.33% (or a disclosed, deliberate rounding of that exact
   value), not "roughly 84%" reconstructed from the model's own sense of the ratio.
5. This applies regardless of how simple the specific ratio looks — a two-number division is
   cheap to compute correctly and expensive to get subtly wrong at scale across a conversation;
   there's no threshold of "simple enough to just say."

### Pre-flight checklist addition
- [ ] Does this answer include any ratio, percentage, growth figure, or pro-rated target? If so,
      was it produced via `code_execution` running the exact formula from §7/the KPI file, and
      does the answer text use that exact printed value rather than a restated/rounded-from-memory
      figure?
